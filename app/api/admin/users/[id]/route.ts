...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { email, name: newName, role: newRole, status: newStatus, assignedCountryIds } = body

  try {
    const updatedUser = await prisma.$transaction(async (tx) => {
      if (assignedCountryIds !== undefined) {
        await tx.userCountryAssignment.deleteMany({
          where: { userId: id },
        });
      }
      ...
      if (assignedCountryIds && Array.isArray(assignedCountryIds)) {
    ...
    if (error.message === 'Invalid role' || error.message === 'Invalid status') {
