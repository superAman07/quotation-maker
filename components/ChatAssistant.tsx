    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed bottom-28 right-8 z-50 flex h-[600px] w-96 flex-col rounded-2xl border border-gray-200 bg-white shadow-2xl"
        >
          {/* Header */}
          ...
          {/* Chat Messages */}
          <ScrollArea className="flex-grow bg-gray-50 p-4" ref={scrollAreaRef}>
            <div className="space-y-4">
              {messages.map(m => (
                <div key={m.id} className={`flex items-end gap-2 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {m.role === 'assistant' && (
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#3e482e] text-white flex-shrink-0">
          ...
          <form onSubmit={handleSubmit} className="flex items-center gap-3 border-t bg-white p-4">
            <Input
              ...
              disabled={isLoading}
              className="flex-grow rounded-full border-gray-300 bg-gray-100 text-gray-700 placeholder-gray-400 focus:ring-2 focus:ring-[#6C733D]"
            />
